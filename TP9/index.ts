...
class MyAlfVisitor extends AbstractParseTreeVisitor<ASTNode> implements AlfVisitor<ASTNode> {
    ...
    visitExpressionFunctionCall(ctx: ExpressionFunctionCallContext): FunctionCallNode {
        let function_call: FunctionCallNode = this.visit(ctx.function_call()) as FunctionCallNode;
        if (function_call !== undefined) {
            return new FunctionCallNode(function_call.function_name, function_call.parameters, function_call.line);
        } else throw new Error();
    }
